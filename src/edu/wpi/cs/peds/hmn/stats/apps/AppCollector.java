...
public class AppCollector {
	 ...
	 * the user. Note, this list should only ever contain a single app.
	 * 
	 * @param packageManager
	 *            used to query the device
	 ...
	 */
	public static List<Application> getActive(PackageManager packageManager,
		...
		PackageInfo activeAppPackage = getPackageInfo(
				activeAppComponent.getPackageName(), packageManager,
				activityManager);
	 ...
	 */
	public static List<Application> getRunningApps(
			PackageManager packageManager, ActivityManager activityManager) {
		Set<String> packageNames = new HashSet<String>();
		for (RunningAppProcessInfo process : activityManager
	 ...
	 */
	public static List<Application> getForegroundApps(
			PackageManager packageManager, ActivityManager activityManager) {
		Set<String> packageNames = new HashSet<String>();
	 ...
	 * @return
	 */
	public static List<Application> getBackgroundApps(
			PackageManager packageManager, ActivityManager activityManager) {
		List<Application> foreground = getForegroundApps(packageManager,
				activityManager);
		List<Application> cached = getCachedApps(packageManager,
				activityManager);

		List<Application> background = getRunningApps(packageManager,
				activityManager);
	 ...
	 */
	public static List<Application> getCachedApps(
			PackageManager packageManager, ActivityManager activityManager) {
		Set<String> packageNames = new HashSet<String>();
		for (RunningAppProcessInfo process : activityManager
				.getRunningAppProcesses())
	 ...
	 */
	private static List<Application> sortedApplicationList(
	 ...
	 * 
	 * @param packageNames
	 *            the list of packages to look up
	 * @param packageManager
	 *            used to query the device
	 * @param activityManager
	 *            aids in determining app state
	...
	private static List<PackageInfo> getPackageInfoList(
			List<String> packageNames, PackageManager packageManager,
			ActivityManager activityManager) {
		List<PackageInfo> packageInfoList = new ArrayList<PackageInfo>();
		for (String packageName : packageNames)
			packageInfoList.add(getPackageInfo(packageName, packageManager,
					activityManager));
	 ...
	 *            a list of packages
	 * @param packageManager
	 *            used to find each package's human-readable name
	 ...
	 * 
	 * @param packageInfo
	 *            the package corresponding to the app
	 * @param packageManager
	 *            used in determining the apps human-readable name
	 * @return an Application representing the given package info
	 */
	private static Application appFromPackage(PackageInfo packageInfo,
